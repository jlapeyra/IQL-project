...
def split(data:list, split_when:Callable[[list], bool], len_sep:int, keep_sep:bool, allow_empty) -> list[list]:
    ret:list[list] = []
    buffer:list = []
    for pos in data:
        buffer.append(pos)
        if len(buffer) >= len_sep and split_when(buffer):
            if not keep_sep:
                buffer = buffer[:-len_sep]
